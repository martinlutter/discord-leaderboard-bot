...
import { APIEmbedField, type APIApplicationCommandInteractionDataUserOption, type APIChatInputApplicationCommandInteraction, type APIInteractionResponseCallbackData, type APIMessageApplicationCommandInteraction } from 'discord-api-types/v10'
...
import { getWeekNumber } from './vote'
...
const execute = async (interaction: APIChatInputApplicationCommandInteraction | APIMessageApplicationCommandInteraction): Promise<APIInteractionResponseCallbackData> => {
  ...
  const userOptionValue = (interaction.data.options?.find(option => option.name === 'user') as APIApplicationCommandInteractionDataUserOption).value
  ...
  for (const weekNumber of weeksInMonth) {
    votes.push(...filterVotesToCurrentMonth(await getVotesByWeekForUser(weekNumber, user.id)))
  }
  ...
  const voteMessages = await Promise.all(
    votes.map(async (vote): Promise<VoteMessage> => {
      const message = await discordApi.channels.getMessage(vote.channelId, vote.messageId)
      return { vote, message }
    })
  )
  ...
  return {
    ...
    embeds: [
      {
        fields: buildVoteEmbeds(groupVoteMessages(voteMessages), interaction.guild_id!)
      }
    ]
  }
}
...
function filterVotesToCurrentMonth(votes: RecordedVote[]): RecordedVote[] {
  ...
  return votes.filter(vote => vote.votedAt > startOfMonth && vote.votedAt < endOfMonth)
}

function groupVoteMessages(voteMessages: VoteMessage[]): GroupedVoteMessage[] {
  const groupedVotes = voteMessages.reduce(
    (acc, voteMessage) => {
...
function buildVoteEmbeds(groupedVoteMessages: GroupedVoteMessage[], guildId: string): APIEmbedField[] {
  return groupedVoteMessages
    .map(grouped => ({
      name: `${grouped.count} vote` + (grouped.count > 1 ? 's' : ''),
      value: `${grouped.voteMessage.message.content.substring(0, 100)}...[Jump to full message](https://discord.com/channels/${guildId}/${grouped.voteMessage.vote.channelId}/${grouped.voteMessage.vote.messageId})`
    }))
    .slice(0, 24)
}